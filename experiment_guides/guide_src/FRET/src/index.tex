...
\begin{equation}
  \ch{F <=> U} 
\end{equation}
The equilibrium constant and free energy of unfolding are then given as 
\begin{align}
...
Here, we are interested in the free energy of unfolding in the absence of denaturant. 
...
Here, \( m \) (\unit{\kilo\cal\per\Molar\per\mole}) describes the rate of change of the free energy of denaturant with respect to denaturant concentration and \( C \) (\unit{\Molar}) is the molar concentration of denaturant.  
...
\begin{equation}
	...
	\label{eq:conc_frac}
\end{equation}
it is relatively straightforward to obtain the following equation:
\begin{equation}
	f = \frac{ \exp\br{ -m \br{\frac{C_\mtext{m} - C}{RT}}}}{1 + \exp\br{-m \br{\frac{C_\mtext{m} - C}{RT}}}} \, .
	\label{eq:fold_frac}
\end{equation}
Experimental data points in an unfolding curve are fit to \cref{eq:fold_frac}, with \( f \) and \( C \) as dependent and independent variables, respectively, to yield values for \( m \) and \( C_\mtext{m} \). 
...
\begin{enumerate}
	...
	\item Prepare a stock solution (\qty{50}{\mL}) of \qty{20}{\milli\Molar} \pH{7.4} phosphate buffer. 
	...
	\item Prepare a stock solution (\qty{50}{\mL}) of \qty{10.0}{\Molar} \iupac{urea} (CAS 75-13-6) in \qty{20}{\milli\Molar} \ch{KP_{\emph{i}}}. 
	...
	\item Prepare a stock solution (\qty{0.8}{\mL} of \qty{\sim500}{\micro\Molar}) of cyt \emph{c} (CAS 9007-43-6) in \ch{KP_{\emph{i}}} (\( \varepsilon_{410} \) for cyt \emph{c} in this solution is \qty{\sim105000}{\per\Molar\per\cm}, \( \varepsilon_{530} \) is \qty{11200}{\per\Molar\per\cm}).
	\label{step:cyt_c_stock}
	\item Prepare several samples of \qty{\sim10}{\micro\Molar} cyt \emph{c} (\qty{1.5}{\mL} each) by mixing the appropriate amounts of stock solution from \cref{step:cyt_c_stock} with stock solutions made in \cref{step:buffer_stock,step:urea_stock} to achieve final urea concentrations of \SIrange{0.0}{10.0}{\Molar} in \qty{1.0}{\Molar} increments. 
	...
	The fluorescence spectra should be recorded with \( \lambda_\mtext{ex} \) set to \qty{290}{\nm} and \( \lambda_\mtext{det} \) should be scanned from \SIrange{305}{500}{\nm}. 
	Use ``high'' sensitivity and \qty{5}{\nm} slits for both excitation and emission. 
	These spectra will be used to determine the free energy of unfolding of cyt \emph{c} using fluorescence changes of Trp-59 as the probe. 
...
\begin{enumerate}
	\item Prepare \qty{5}{\mL} of a \qty{\sim10}{\micro\Molar} solution of the tryptophan model compound, \iupac{\N-acetyl-tryptophanamide} (NATA, CAS 2382-79-8) in \ch{KP_{\emph{i}}}. 
...
\section{Part C: Data Analysis} % (fold)
...
\begin{enumerate}
	...
	\label{step:abs_fit}
	\item Use the fluorescence spectrum collected in \cref{step:nata_spectrum} along with the absorption spectrum of \qty{\sim10}{\micro\Molar} cyt \emph{c} (\qty{0.0}{\Molar} \iupac{urea}) from \cref{step:spec_fit} to calculate the Förster distance of the Trp--heme pair. 
...
% section part_c_data_analysis (end)

\section{Questions and Further Thoughts} % (fold)
\label{sec:questions_and_further_thoughts}

\begin{enumerate}
	\item What is the source of the large peak at \qty{\sim323}{\nm} in the spectra of the buffers (\ch{KP_{\emph{i}}} and \iupac{urea}/\ch{KP_{\emph{i}}}) as well as in some of the cyt \emph{c} samples?
	...
	\item Compare the distance between Trp-59 and the heme group for the fully folded protein (\qty{0.0}{\Molar} \iupac{urea}) with distances obtained from the known crystal structure (\href{https://www.rcsb.org/structure/1hrc}{PDB: 1HRC}). 
