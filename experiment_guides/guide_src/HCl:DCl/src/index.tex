%TODO 
% - Check for updated libraries (cclib, etc.)
...
The infrared gas cell is constructed from a short (\qtyrange{5}{10}{\cm}) length of large-diameter (\qtyrange{4}{5}{\cm}) Pyrex tubing with a vacuum stopcock attached. 
...
Prepare an indicator solution by dissolving \numrange{3}{4} drops of bromcresol green in \qty{300}{\mL} of DI water in a \qty{400}{\mL} beaker. 
...
%
% With a disposable plastic pipette, obtain \qty{\sim 1}{\mL} of concentrated \ch{H2SO4}.
...
I recommend scanning from \qtyrange{0.6}{4.5}{\angstrom} in steps less than \qty{0.05}{\angstrom}. 
...
Note the addition of the \lstinline[language=gaussian]{(Iso=<mass-num>)} option after the element labels.\sidenote{i.e., \lstinline[language=gaussian]{<atom1>(Iso=<mass_num>)} should become \lstinline[language=gaussian]{He(Iso=3)} for a \ch{^3He} atom)} 
...
In this same section will be equilibrium rotational constants for the two degenerate rotational modes (labeled \Verb|Ba(y)| and \Verb|Ca(z)|) and the energy (relative to the zero-point energy) of the ground state \( (D_0) \).\sidenote{All energy values are listed in \unit{\wn}.}
...
\begin{enumerate}
	...
	\item Using your value of \( \widetilde{\nu}_0 \) for \ch{HCl}, calculate \( \widetilde{C}_v\br{\mtext{vib}} \) at \qty{298}{\K} and at \qty{1000}{\K} from \cref{eq:vib_part_func2}. 
	Compare the spectroscopic value \( \widetilde{C}_v = 5/2 R + \widetilde{C}_v(\mtext{vib}) \) with the experimental \( \widetilde{C}_v \) value obtained from directly measured values~\autocite{lewis61,spencer48} of \( \widetilde{C}_p \) and the expression \( \widetilde{C}_v = \widetilde{C}_p - R \): \( \widetilde{C}_v = \qty{20.80}{\J \per \mole\K} \) at \qty{298}{K} and \qty{23.20}{\J \per \mol\K} at \qty{1000}{\K}. 
\end{enumerate}
