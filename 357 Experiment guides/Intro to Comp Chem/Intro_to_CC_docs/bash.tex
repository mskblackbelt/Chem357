...
\begin{itemize}
  ...
  \begin{description}[
    ...
    style=nextline,
    itemindent=*]
    \item[grep "someText" "file1"] search for the text ``someText'' in ``file1''.\sidenote{If your input has spaces, enclosing the input in double quotes (\Verb{"}) will preserve the spaces, \emph{e.g.,}\Verb{"}Some quoted text\Verb{"}.} The \Verb{-i} flag tells grep to \textbf{i}gnore letter case (upper/lower).
  ...
  \begin{description}[
    ...
    style=nextline,
    itemindent=*]
    \item[>] at the end of a command to redirect the result to a file
    \item[>>] at the end of a command to append the result to the end of a file
