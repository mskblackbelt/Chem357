...
This file starts a calculation on two processor cores using \SI{400}{\mega\byte} of memory.
...
\begin{description}[font=\ttfamily\upshape]
  ...
  \item[atomX <X> <Y> <Z>] This block specifies the geometry of the system.
    You can use either the atomic symbol (e.g., \Verb{C}) or the atomic number (e.g., \Verb{6}) to specify the atom type, followed by it's cartesian coordinates in units of Angstroms (\si{\angstrom}).\sidenote{
