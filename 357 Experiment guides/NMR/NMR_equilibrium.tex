...
\begin{marginfigure}
  \centering
   \input{figures/zeeman_splitting.tikz}
...
\begin{figure}[htb]
  ...
  \caption{Energy levels, transitions, and the NMR spectrum for acetaldehyde (\ch{CH3CHO}) at \SI{100}{\MHz}. 
...
In \iupac{\N,\N-dimethylacetamide}, the two \iupac{\N-methyl} groups can undergo chemical exchange through a process analogous to a \emph{cis--trans} isomerization, where the product is chemically identical to the reactant.
\begin{figure}[htb]
	\centering
		\schemestart
...
\subsubsection{Arrhenius and Eyring equations} % (fold)
...
% subsubsection arrhenius_and_eyring_equations (end)
...
Usually, spin--spin and spin--lattice relaxation determine the width of an NMR absorption peak.\autocite{pople67}
...
\subsection{Approximate Methods for Evaluation of Rate Constants\autocite{johnson66}} % (fold)
...
\subsubsection{Slow and Intermediate Exchange} % (fold)
...
% subsubsection slow_and_intermediate_exchange (end)

\subsubsection{Coalescence Temperature} % (fold)
...
% subsubsection coalescence_temperature (end)

\subsubsection{Fast Exchange} % (fold)
...
\end{equation}
% subsubsection fast_exchange (end)
...
\begin{enumerate}
	\item Prepare a \SI{15}{vol\percent} solution of \ch{DMA} and \ch{DMSO-d6} in  a \SIrange{1.5}{2.0}{\mL} microfuge tube. 
