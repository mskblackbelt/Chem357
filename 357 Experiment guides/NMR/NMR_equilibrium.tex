...
\end{figure}
Due to the characteristic period of the \NMR* measurement, a large range of reaction rates relevant in the chemical laboratory is easily accessible (\SIrange{1e-1}{1e-5}{\per\second}).
In addition, rotational barriers in the range of \SIrange{3}{20}{\kcal\per\mol} can be studied by this method.\autocite{bovey69} 
...
If two groups of chemically equivalent nuclei are exhanged by an intramolecular process, the \NMR* spectrum is a function of the difference in their resonance frequencies, \( \nu_{\ch{A}} - \nu_{\ch{B}} = \increment\nu \), and of the rate of exchange, \( k \).\sidenote{A typical value for \( \increment\nu \) is about \SI{10}{\hertz}.} 
