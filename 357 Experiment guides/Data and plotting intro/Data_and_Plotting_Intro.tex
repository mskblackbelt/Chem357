...
Begin by downloading the input files from the \Verb{Plotting Tutorial} folder on Blackboard. 
This folder contains a few datasets in various formats. 
...
Sometimes, a \Verb{.dat} extension might be used for a plain-text file to indicate `data' regardless of the format or delimiter type. 
...
On a Mac, you can also select \textbf{File \blacktriangleright Import} in the menu bar.
...
Return to your data spreadsheet and highlight a \( 2 \times 5  \) grid of empty cells (the cells directly under or adjacent to your dataset will do). 
...
The first range should be your \( y \)-values (cells \textbf{B2} through \textbf{B22}), while the second range is your \( x \)-values (\textbf{A2} through \textbf{A22}).
...
From the set of input files you downloaded from Blackboard, open the file labeled \Verb{FTIR_UVVis_template.xltx}.\footnote{This template is handy to keep around for future work.} 
Copy the data from the FTIR sheet and paste it into your plotting document in a new sheet called \textbf{FTIR}. 
...
Label column \textbf{A} ``Wavenumber (\si{\cm^{-1}})'', the new column \textbf{B} as ``Wavelength (\si{\um})'', column \textbf{C} as ``\% Transmission'', and column \textbf{D} as ``Absorbance''.\footnote{What are the relevant units for \%T and A?}
