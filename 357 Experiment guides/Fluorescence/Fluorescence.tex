...
The units shown (and factor of 1000) pertain if \( C_0 \) is expressed in molarity (\unit{\mol\per\liter}) and \( R \) and \( D \) are expressed in \unit{\cm} and \unit{\cm\squared \per \s}, respectively.\sidenote[][-5\baselineskip]{
To convert from \unit{molecules} to \unit{\Molar}, we need to acount for the number of particles in a \unit{mole} (Avogadro's number) and convert from our molecular length units (\unit{\cm}) to proper volume units. It's useful to know that \( \qty{1}{\L} =\qty{1}{\dm\cubed} \).}
...
\begin{tabular}{lr}
  ...
  \( \lambda_\mtext{det} \) & \qtyrange{350}{550}{\nm}\\
  Scan speed & \qty{300}{\nm\per\min}\\
  Slit width & \qty{2.5}{\nm}\\
  \bottomrule
\end{tabular}}
...
\begin{enumerate}
	\item Tabulate your data as \( I \) (the anthracene fluorescence at \num{398} or \qty{399}{\nm}) and \ch{[Q]} (the \ch{CBr4} concentration, in \unit{\Molar}).\marginnote{This is a good place to practice with the Pandas library. 
