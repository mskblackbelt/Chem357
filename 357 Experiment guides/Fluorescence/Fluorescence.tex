...
\begin{abstract}
\noindent
The objective of this lab is to determine the rate constant and collision diameter of a diffusion-controlled reaction (photoexcited anthracene with carbon tetrabromide, \ch{CBr4}) using the technique of fluorescence quenching. \thanks{Transcribed (with corrections) from \cite{halpern97}}
\end{abstract}

%\printclassoptions


\section{Introduction test} % (fold)
\label{sec:intro}
...
\begin{equation}
	...
	\label{eq:fickBa}
\end{equation}
Because we are concerned with ta three-dimensional and isotropic space, \emph{i.e.,} equal forces in all directions, we can write Fick's second law in a more general form as 
\begin{equation}
...
\begin{equation}
	k_1 = 4\pi R D N_A \br{ 1 + \frac{R}{\sqrt{\pi D t}}} \qquad \si{\per\Molar \per\s} \, .
	\label{eq:rateconst1}
\end{equation}
The units shown pertain if \( R \) and \( D \) are expressed in \si{\cm} and \si{\cm\squared \per \s}, respectively. 
This rate constant is not a true \emph{constant}, however, because of the transient term. 
However, as \( t \) becomes large enough, 
\begin{equation}
	\lim_{t \to \infty} k_1 = 4\pi R D \qquad \si{\per\Molar \per\s} \, ,
	\label{eq:rateconst2}
\end{equation}
...
\sidenote[][-2.0in]{An important consideration in \cref{eq:rateconst1,eq:rateconst2} is the time dependence of \( k_1 \). 
...
\sidenote{This is the \emph{only} equation in which \( \overline{R} \) refers to the gas constant. 
...
The indicated units are obtained if \( \overline{R} \) and \( \eta \) are expressed in SI units, such that \( \overline{R} = \SI{8.314}{\kJ \per \mol \per \K} \) and \( \eta = \si[inter-unit-product=\ensuremath{{}\cdot{}}]{\Pa \s} \).
...
The time dependence was treated by Ware and Novros\cite{ware66}, who applied \cref{eq:rateconst1} to the condition pertinent to this experiment, namely, that the system is considered to be suddenly exposed to a steady-state excitation source, called a step function (see \cref{fig:step_func} and Further Readings). 
...
in which 
\begin{align*}
	a &= \frac{1}{\tau_0} + 4 \pi R D N_A \ch{[Q]} \, ,\\
	b &= 4 R^2 \sqrt{\pi D} N_A \ch{[Q]}\, .
\end{align*}
...
\begin{enumerate}
	...
	Use the proper units as described in the manual. Once you have \( k_1 \), insert it into \cref{eq:rateconst2} to calculate the collision radius \( R \) (don’t mix up your \( R \)s).\sidenote{
