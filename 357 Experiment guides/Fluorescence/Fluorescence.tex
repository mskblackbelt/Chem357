...
To convert from \unit{molecules} to \unit{\Molar}, we need to account for the number of particles in a \unit{mole} (Avogadro's number) and convert from our molecular length units (\unit{\cm}) to proper volume units. It's useful to know that \( \qty{1}{\L} =\qty{1}{\dm\cubed} \).}
...
\begin{enumerate}
	\item Tabulate your data as \( I \) (the anthracene fluorescence at \num{398} or \qty{399}{\nm}) and \ch{[Q]} (the \ch{CBr4} concentration, in \unit{\Molar}).\marginnote[-2\baselineskip]{This is a good place to practice with the Pandas library. 
  ...
  Once you have \( k_1 \), insert it into \cref{eq:rateconst2} to calculate the collision radius \( R \) (don’t mix up \( \overline{R} \) and \( R \)).\sidenote[][-7\baselineskip]{
