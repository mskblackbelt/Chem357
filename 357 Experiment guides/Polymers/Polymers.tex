...
\begin{abstract}
\noindent
The objective of this lab is to determine the viscosity-average molecular weight of poly(vinyl alcohol) (PVOH) and the fraction of ``head-to-head'' monomer linkages in the polymer.\thanks{Transcribed (with corrections) from \cite{halpern97}} % Most of text available at https://books.google.com/books?id=Pe0J-vDm_TMC&pg=PT392
\end{abstract}
...
\begin{figure}
	...
	\begin{tikzpicture}[scale=1.5]
		\draw[thick]	(-2,0)	--	(4.6,0);
		\draw[thick]	(-2,3.2)	--	(4.6,3.2);
		...
		\draw[thick,->]	(-1,-0.15)	--	(1,-0.15) node[below]{$x$}	--	(3,-0.15);
		\draw[<-]	(I)	--	(J);
		\draw[->]	(J)	--	(K);
...
This relationship was first expressed quantitatively by Arrheius (1912):\sidenote{Arrhenius is also associated with a similar equation that expresses the temperature dependence of a rate constant.}
\begin{equation}
