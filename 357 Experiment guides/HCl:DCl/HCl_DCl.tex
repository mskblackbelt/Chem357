...
Both types of salt windows become ``foggy'' on prolonged exposure to a moist atmosphere and should be protected\sidenote{\emph{e.g.,} stored in a dessicator} when not in use. 
...
Prepare the air condenser by placing a plug of glass wool at the bottom and filling the rest with \ch{CaCl2} (approximately 12 mesh). 
...
\tcbinputlisting{
	title=Contents of Script \texttt{run\_scan.sh},
	fonttitle=\bfseries,
	listing only,
	...
	minted options={
		...
		obeytabs,
		tabsize=4},
	grow to right by=9em,
	listing file=Gaussian_simulations/potential_curves/run_scan.sh}
%
Once you have calculated the potential energy surface for the molecule, perform an optimization and frequency analysis of the \ch{HCl} molecule.\sidenote{As outlined for \ch{H2O} in Problem 3 in the Introduction to Computational Chemistry lab.} 
You only need to do this for \ch{H^{35}Cl} and \ch{D^{35}Cl}. 
...
\tcbinputlisting{
	...
	overlay={
		...
		\end{tcbclipinterior}},
	listing only,
	listing file=Gaussian_simulations/potential_curves/template_input.temp}
%
...
You will need to replace all items in angle brackets  with appropriate data.\sidenote{\emph{i.e.,} ``\lstinline`<atom1>(Iso=<mass_number>)`'' should become ``\lstinline`He(Iso=3)`'' for a \ch{^3He} atom)}
...
\begin{enumerate}
	...
	If the \ch{^{35}Cl}/\ch{^{37}Cl} splitting is seen, index the stronger \ch{^{35}Cl} lines. 
