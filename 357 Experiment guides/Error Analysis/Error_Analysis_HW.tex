...
\begin{enumerate}
	\item In each of the following cases, determine the answer and its error, assuming that the errors to the relevant quantities involved in the calculation are uncorrelated (\emph{i.e.,} the errors add in quadrature).\marginnote{This problem is from \autocite{lyons1991data}.}
	\begin{enumerate}
	...
	\item\marginnote{The following problems come from \autocite{taylor1997error}.} Evaluate each of the following:
	\begin{enumerate}
		\item \( \br{\num{5.6(7)} + \num{3.70(3)}} \)
	...
	\item If an object is placed at a distance \( p \) from a lens and an image is formed at a distance \( q \) from the lens, the lens's focal length can be found as\sidenote{This equation follows from the ``lens equation'', \( 1/f = (1/p) + (1/q) \).} 
	\begin{equation}
		f = \frac{pq}{p + q} \, . 
		\label[equation]{eq:lens}
	\end{equation}
	\begin{enumerate}
		\item \label[problem]{prob:lens_a} Use the general rule (3.47) to derive a formula for the uncertainty \( \delta f \) in terms of \( p \), \( q \), and their uncertainties. 
